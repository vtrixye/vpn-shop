...
@remnawave_handler("user.revoked")
async def user_revoked(session: AsyncSession, user: UserDto):
    sub = await session.get(Subscription, user.uuid)
    if sub is not None:
...
async def user_expiration(session: AsyncSession, user: UserDto, meta: dict):
